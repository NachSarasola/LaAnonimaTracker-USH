...
class LaAnonimaScraper:
    ...
    def _apply_branch_via_cp_query(self, postal_code: str) -> bool:
        ...
        except BranchSelectionError as exc:
            logger.warning("CP query branch probe failed: {}", exc)
            return False
        if is_set:
    ...
    def select_branch(self) -> bool:
        ...
        if self.branch_selection_strategy in {"cp_query_first", "auto"}:
            ...
            # Recover to homepage before modal fallback in case cp query landed on an error page.
            self.page.goto(
                self.base_url,
                wait_until="domcontentloaded",
                timeout=self.navigation_timeout,
            ...
            is_set, current = self.check_branch_set()
            if is_set:
                logger.info(f"Branch verified after CP query fallback reload: {current}")
                self.current_branch = current
                return True
